...
from utils.time_analysis import get_weekly_free_slots, get_class_suggestions, get_worked_hours_analysis
import logging
...
def register_routes(app):
    ...
    def new_lesson():
        ...
        if date_str:
            try:
                prefill_data['date'] = datetime.strptime(date_str, '%Y-%m-%d').date()
            except:
                pass
        ...
        if start_time_str:
            try:
                prefill_data['start_time'] = datetime.strptime(start_time_str, '%H:%M').time()
            except:
                pass
        ...
        if end_time_str:
            try:
                prefill_data['end_time'] = datetime.strptime(end_time_str, '%H:%M').time()
            except:
                pass
        ...
        if student_id:
            prefill_data['student_id'] = int(student_id)
        
        students = Student.query.all()
        return render_template('forms/lesson_form.html', students=students, prefill=prefill_data)
    ...
    # Análise de gestão de tempo e sugestões de aulas
    @app.route('/time-analysis')
    def time_analysis():
        # Obter parâmetros da URL
        start_date_str = request.args.get('start_date')
        days = int(request.args.get('days', 7))  # Padrão: 7 dias
        view = request.args.get('view', 'disponibilidade')  # disponibilidade ou horas_trabalhadas
        ...
        # Obter análise de tempo livre
        free_time_data = get_weekly_free_slots(start_date, days)
        
        # Obter análise de horas trabalhadas
        worked_hours_data = get_worked_hours_analysis(start_date, days)
        
        # Obter sugestões de aulas
        subjects = ["Matemática", "Física", "Química", "Inglês"]
        suggestions = get_class_suggestions(start_date=start_date, days=days, subjects=subjects)
        
        # Calcular estatísticas para o gráfico de disponibilidade
        daily_stats = {}
        for slot in free_time_data['free_slots']:
            day_key = slot['date'].strftime('%Y-%m-%d')
            if day_key not in daily_stats:
                daily_stats[day_key] = {
                    'date': slot['date'],
                    'day_name': slot['day_name'],
                    'free_hours': 0,
                    'total_work_hours': free_time_data['total_work_hours'] / len(free_time_data['free_slots']) if free_time_data['free_slots'] else 0,
                    'percent_free': 0
                }
            
            daily_stats[day_key]['free_hours'] += slot['duration_hours']
            
        # Calcular porcentagem para cada dia
        for day_key, stats in daily_stats.items():
            stats['percent_free'] = (stats['free_hours'] / stats['total_work_hours'] * 100) if stats['total_work_hours'] > 0 else 0
            
        # Ordenar por data
        daily_stats = dict(sorted(daily_stats.items()))
        
        # Agrupar sugestões por dia
        suggestions_by_day = {}
        for suggestion in suggestions:
            day_key = suggestion['slot']['date'].strftime('%Y-%m-%d')
            if day_key not in suggestions_by_day:
                suggestions_by_day[day_key] = []
            suggestions_by_day[day_key].append(suggestion)
        
        return render_template(
            'time_analysis.html',
            free_time_data=free_time_data,
            worked_hours_data=worked_hours_data,
